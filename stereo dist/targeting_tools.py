...
class Frame_Motion:
    ...
    # True = return (x,y,bx,by,bw,bh), else check target_return_size
    target_return_box = False
    # True = return (x,y,percent_frame_size), else just (x,y)
    ...
    def targets(self, frame):
        ...
        # blur
        frame2 = cv2.GaussianBlur(
            frame2, (self.gaussian_blur, self.gaussian_blur), 0)
        ...
        # threshold
        frame3 = cv2.threshold(frame3, self.threshold,
        ...
        if self.contour_draw:
            for size, x, y, bx, by, bw, bh, c in targets:
                cv2.drawContours(
                    frame, [c], 0, self.contour_color, self.contour_line)
        ...
        if self.contour_box_draw:
            for size, x, y, bx, by, bw, bh, c in targets:
                ...
                cv2.circle(frame, (bx+int(bw/2), by+int(bh/2)), self.contour_box_point,
                           self.contour_box_color, self.contour_box_pline)
...
class Frame_Angles:
    ...
    def build_frame(self):
        ...
        self.x_adjacent = self.x_origin / \
            math.tan(math.radians(self.angle_width/2))
        self.y_adjacent = self.y_origin / \
            math.tan(math.radians(self.angle_height/2))
    # ------------------------------
    ...
    def frame_add_crosshairs(self, frame):
        ...
        cv2.line(frame, (0, self.y_origin),
                 (self.pixel_width, self.y_origin), (0, 255, 0), 1)
        cv2.line(frame, (self.x_origin, 0),
                 (self.x_origin, self.pixel_height), (0, 255, 0), 1)
        cv2.circle(frame, (self.x_origin, self.y_origin), int(
            round(self.y_origin/8, 0)), (0, 255, 0), 1)
        ...
        for angle in range(10, 95, 10):
            ...
            if x <= self.x_origin:
                cv2.line(frame, (self.x_origin-x, 0),
                         (self.x_origin-x, self.pixel_height), (255, 0, 255), 1)
                cv2.line(frame, (self.x_origin+x, 0),
                         (self.x_origin+x, self.pixel_height), (255, 0, 255), 1)
            # draw horizontals
            if y <= self.y_origin:
                cv2.line(frame, (0, self.y_origin-y),
                         (self.pixel_width, self.y_origin-y), (255, 0, 255), 1)
                cv2.line(frame, (0, self.y_origin+y),
                         (self.pixel_width, self.y_origin+y), (255, 0, 255), 1)
